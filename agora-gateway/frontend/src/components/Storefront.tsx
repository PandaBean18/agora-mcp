...
export function Storefront({ products }: { products: any[] }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {products.map((p) => (
        <div key={p.agora_sku} className="glass-panel p-5 rounded-xl border border-slate-700/50 hover:border-indigo-500/50 transition-colors">
          <div className="flex justify-between items-start mb-3">
            <h3 className="font-semibold text-slate-200 text-lg">{p.name}</h3>
            <span className="bg-emerald-500/10 text-emerald-400 font-mono text-sm px-2.5 py-1 rounded-md border border-emerald-500/20">
